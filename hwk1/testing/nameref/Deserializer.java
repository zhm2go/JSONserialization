...
public class Deserializer{
	...
	public static Faculty real_readFaculty(JSONObject js, HashMap<Integer, Object> hmap) throws JSONException{
		...
		for(int courses_j = 0;courses_j < all_fs.length();courses_j++){
			JSONObject p_f = all_fs.getJSONObject(courses_j);
			if(p_f.has("courses")){
				JSONArray courses_ar = p_f.getJSONArray("courses");
				for(int courses_l = 0; courses_l < courses_ar.length();courses_l++){
					JSONObject courses_f = courses_ar.getJSONObject(courses_l);
					if(courses_f.has("ref")){
						Course y = (Course)hmap.get("ref");
						x.addCourses(y);
					}
					else{
						Course z = real_readCourse(courses_f, hmap);
						x.addCourses(z);
					}
				}
			}
